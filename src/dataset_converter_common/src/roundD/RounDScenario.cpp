...
void RounDScenario::MakePathsAbsolute(const std::string &dataset_root_directory) {
...
void RounDScenario::ParserTrackMetaFile() {
  ...
  for (csv::CSVRow &row : csv_reader) {
    ...
    } else if (class_string == "van") {
      type = cpm_scenario::ExtendedObjectType::VAN;
    } else if (class_string == "trailer") {
      type = cpm_scenario::ExtendedObjectType::TRAILER;
    } else if (class_string == "truck" || class_string == "bus") {
      type = cpm_scenario::ExtendedObjectType::TRUCK_BUS;
    } else if (class_string == "bicycle" || class_string == "motorcycle") {
